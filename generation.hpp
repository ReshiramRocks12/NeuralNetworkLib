...
public:
	...
	std::shared_ptr<Generation> createNewGeneration(
		...
		Generation::SelectionAlgoritm selectionAlgo = FITNESS_PROPORTIONATE_SELECTION, double selectionPercent = 0.5, double kPercent = 0.2,
...
private:
	...
	std::vector<unsigned int> fitnessProportionateSelection(double selectionPercent);
	std::vector<unsigned int> tournamentSelection(double selectionPercent, double kPercent = 0.2);

	void uniformCrossover(double crossOverRate);
	// void kPointCrossover(double crossOverRate, unsigned int k);
